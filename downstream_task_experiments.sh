...
do
    ...
    do
        python src/weighting_experiment.py --dataset $DATASET  --sample_weighting_method $SAMPLE_WEIGHTING_METHOD  \
        --bias_type mean_difference --experiment_name downstream_task --bias_fraction 0.8 \
            --drop $DROP --n_cv_repeats $N_CV_REPEATS --n_cv_splits $N_CV_SPLITS  --load_previous_results
    done
done
...
DROP=1
for SAMPLE_WEIGHTING_METHOD in fw-mrs-temperature mrs-forest uniform soft-mrs-linear soft-mrs-exponential fw-mrs-temperature-svm
do
